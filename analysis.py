...
def main3():
    ...
    results = sweep_model_hparams( N=N, t_disc=t_disc, d_model_list=d_model_list, nhead_list=nhead_list, num_layers_list=num_layers_list, 
                                  dim_f_list=dim_f_list, dropout=0.1, max_epochs=max_epochs,)

    
    print_model_sweep_table(results)

    # Example: you could also reuse plot_loss_curves for a few configs:
    # pick the best result and plot its curves
